...
def emit(stage, duration_ms, **extra):
    """Emit a completed measurement event (duration already known)."""
    if not ENABLED:
        return
    ev = {"ts": time.time(), "gen_id": _ctx["gen_id"], "step": _ctx["step"],
          "total_steps": _ctx["total_steps"], "layer": _ctx["layer"], "stage": stage,
          "kernel": _ctx["kernel"], "duration_ms": round(float(duration_ms), 4),
          "seq_len": _ctx["seq_len"], "dtype": _ctx["dtype"], "gpu_id": GPU_ID, **extra}
    _send_q.append(ev)
...
class stage:
    ...
    def __init__(self, name, **extra):
        self.name = name; self.extra = extra; self.s = self.e = None
    def __enter__(self):
        if not ENABLED:
            return self
        import torch
    ...
    def __exit__(self, *a):
        ...
        if DEBUG:
            ...
            emit(self.name, self.s.elapsed_time(self.e), **self.extra)
            _evt_pool.append((self.s, self.e))
        else:
            with _lock:
                _pending.append((self.s, self.e, dict(self.extra), self.name))
        return False
    ...
    while True:
        ...
        for _ in range(n):
            with _lock:
                if not _pending:
                    break
                s, e, extra, name = _pending.popleft()
            try:
                if e.query():                       # completed -> safe to read, no sync
                    emit(name, s.elapsed_time(e), **extra)
                    _evt_pool.append((s, e))
                else:
                    with _lock:
                        _pending.append((s, e, extra, name))
            except Exception:
                pass
